...
# =============================================
# CONFIG — Environment Variable မှ ဖတ်ယူမည်
# =============================================
BOT_TOKEN = os.getenv("TELEGRAM_BOT_TOKEN", "8992896661:AAGRRhUul9nZLJL8DkEENuSIwnd-FawFafo")
ADMIN_ID = os.getenv("ADMIN_ID", "8937162965")
...
async def cmd_status(message):
    ...
    await bot.reply_to(
        message,
        f"📊 Scan Status {running}\n\n"
        f"⏱ Time: {elapsed:.1f}s\n"
        f"🔍 Tried: {stats['tried']}\n"
        f"🎯 Current: {stats['current_code'] or '---'}\n"
        f"🟢 Hits: {stats['hits']}\n"
        f"🔴 Expired: {stats['expired']}\n"
        f"🟣 Limits: {stats['limits']}\n"
...
async def cmd_stop(message):
    ...
    if task and not task.done():
        task.cancel()
    await bot.reply_to(message, "🛑 scan ရပ်တန့်ပြီးပါ။")
...
async def main():
    ...
    if not _HAS_OCR:
        print("[!] ddddocr မရှိပါ — OCR feature ပိတ်ထားပါမည်။")
    else:
